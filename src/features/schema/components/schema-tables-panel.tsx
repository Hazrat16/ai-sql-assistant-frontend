...
export function SchemaTablesPanel() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2 px-1">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] text-zinc-500 dark:text-zinc-400">Public tables</span>
        <Button
          type="button"
          variant="ghost"
          size="sm"
      ...
      {usedFallback && error ? (
        <p className="rounded-md border border-amber-200 bg-amber-50 px-2 py-1.5 text-[11px] text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-100">
              ...
              return (
                <li key={t.name} className="rounded-lg border border-zinc-200/80 dark:border-zinc-800">
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-2 px-2 py-2 text-left text-xs font-medium text-zinc-900 hover:bg-zinc-50 dark:text-zinc-50 dark:hover:bg-zinc-900"
                  >
                    <span className="truncate">{t.name}</span>
                    <span className="shrink-0 text-[10px] text-zinc-500">{t.columns.length}</span>
                    <Chevron rotated={expanded} />
                  </button>
                  {expanded ? (
                    <div className="border-t border-zinc-100 dark:border-zinc-900">
                      <ul className="px-2 py-2 text-[11px]">
                        {t.columns.map((c) => (
...
function Chevron({ rotated }: { rotated: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={`h-3.5 w-3.5 shrink-0 text-zinc-500 transition-transform ${rotated ? "rotate-180" : ""}`}
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      aria-hidden
