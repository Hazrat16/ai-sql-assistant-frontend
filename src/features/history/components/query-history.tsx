  ...
  return (
    <Card className="flex min-h-0 flex-1 flex-col overflow-hidden border-zinc-200/80 dark:border-zinc-800">
      ...
      <CardContent className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
        {history.length === 0 ? (
          <p className="text-xs text-zinc-500 dark:text-zinc-400">Saved queries appear here for quick re-run.</p>
        ) : (
          <ul className="space-y-2">
            {history.map((h) => (
              <li
                key={h.id}
                className="rounded-lg border border-zinc-200/80 p-2 text-xs dark:border-zinc-800"
              >
